  ...
  const handlePagar = async () => {
    try {
      ...
      const response = await axios.post(
        `${VITE_APP_URL}/ventasimple`,
        ventaSimple
      );
      console.log("venta simple", response);
      window.alert("tu Compra fue realizada con éxito  ");
      setCart([]);
      navigate("/");
    } catch (error) {
            ...
            <div className='sub-container-cart' key={producto.p_name}>
              ...
              <div className='d-flex align-items-center '>
                <div className='me-5 text-primary  '>
                  <h3>
                    {(producto.p_precio * producto.qty).toLocaleString(
                      "es-CL",
                      {
                        style: "currency",
                        currency: "CLP",
